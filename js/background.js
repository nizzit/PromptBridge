...
// Import utils.js for Manifest V3 service worker
    ...
    chrome.contextMenus.removeAll(() => {
        ...
        storage.sync.get(['prompts'], function (result) {
            ...
            if (prompts.length > 0) {
                ...
                // Add a separator before the settings menu item, but only when text is selected.
                chrome.contextMenus.create({
                    id: 'separator',
                    parentId: 'promptbridge-parent',
                    type: 'separator',
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    // Handles the request to open the options page.
    if (request.action === 'openOptions') {
        chrome.runtime.openOptionsPage();
    }
    // Handles the request to call the API from a content script.
